<template>
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" @submit.prevent="save" class="row" enctype="multipart/form-data">
...
<script>
    export default {
        ...
        computed: {
            listUrl() {
                return baseURL + 'user/expenditure/'
            }
        },
        ...
        methods: {
            onFileChange(e){
                ...
                this.expense.media = e.target.files[0];
            },
            getBankAccount(id) {
                this.$awn.asyncBlock(
            ...
            save() {
                ...
                formData.append('media', this.expense.media);
                formData.append('date', this.expense.date);
                formData.append('amount', this.expense.amount);
                formData.append('where', this.expense.where);
                formData.append('gl_account_id', this.expense.glAccount);
                formData.append('gl_account_head_id', this.expense.glAccountHead);
                formData.append('cash_id', this.expense.cash.id);
                formData.append('bank_id', this.expense.bank.id);
                formData.append('bank_account_id', this.expense.bank.account);
                formData.append('note', this.expense.description);
                this.$awn.asyncBlock(
                    axios.post(baseURL + 'user/expenditure', formData, config),
                    response => {
                        // flase message
