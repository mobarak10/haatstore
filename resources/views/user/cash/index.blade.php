...
@section('content')
    <div class="container">
        ...
        <div class="row pt-2 pb-5">
                ...
                <div class="col-md-4 mb-3">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('cash.show', $cash->id) }}" class="btn btn-success text-white" title="Cash details.">
                                <i class="fa fa-eye" aria-hidden="true"></i>
                            </a>

                            {{--@can('edit', App\Models\Cash::class)--}}
                                <a href="{{ route('cash.edit', $cash->id) }}" class="btn btn-success" title="Change cash information.">
                                    <i class="fa fa-pencil" aria-hidden="true"></i>
                                </a>
                            {{--@endcan--}}

                            <a href="{{ route('cash.index') }}" class="btn btn-danger float-right" title="Trash" onClick="if(confirm('Are you sure, You want to delete this record?')){event.preventDefault();document.getElementById('delete-form-{{ $cash->id }}').submit();} else {event.preventDefault();}">
                                <i class="fa fa-trash" aria-hidden="true"></i>
                            </a>

                            <form action="{{ route('cash.destroy', $cash->id) }}" method="post" id="delete-form-{{ $cash->id }}" style="display: none;">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                            </form>
                        </div>
            ...
            {{--@can('create', App\Models\Cash::class)--}}
                <div class="col-md-4">
