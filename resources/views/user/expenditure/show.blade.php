                        ...
                        <tbody>
                            ...
                            @foreach($expenditures as $expenditure)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($expenditure->media)
                                            <a href="{{ asset(($expenditure->media) ? $expenditure->media->real_path : '') }}" target="_blank">
                                                View Attachment
                                            </a>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-right print-none">
                                        <a class="btn btn-primary" href="{{ route('expenditure.edit', $expenditure->id) }}" target="_blank" title="Show all expense in this date.">
                                            <i class="fa fa-pencil" aria-hidden="true"></i>
                                        </a>

                                        <a href="{{ route('expenditure.destroy', $expenditure->id) }}" class="btn btn-danger" title="Trash" onClick="if(confirm('Are you sure, want to delete this record?')){event.preventDefault();document.getElementById('delete-form-{{ $expenditure->id }}').submit();} else {event.preventDefault();}">
                                            <i class="fa fa-times" aria-hidden="true"></i>
                                        </a>

                                        <form action="{{ route('expenditure.destroy', $expenditure->id) }}" method="post" id="delete-form-{{ $expenditure->id }}" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
